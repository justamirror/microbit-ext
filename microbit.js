...
class microbit {
	...
	#maincallback(reason, device, data) {
		if (reason === 'connected') {
			let id = device.productName.slice(15, -1)
			this._devices[id] = {'id':id,'send':(text)=>{
				uBitSend(device, text)
			},'output':[],'disconnect':()=>{
		...
		if (data === null) {
		    o = {
				...
				data: null,
				timeStamp: null,
				device: this.device(device.productName.slice(15, -1))
			}
		} else if (reason.startsWith('graph-')) {
		    o = {
				microbit: this,
				dataType: reason,
				graph: data.graph,
		...
		if (reason === 'disconnected') {
			delete this._devices[o.device.id]
		}
		
		o.device.output.push(o)
		this._cbs.forEach((cb)=>{
			cb(o)
		})
	}
	connect() {
		let mcb = this.#maincallback
		mcb = mcb.bind(this)
	...
	on(cblist, cb, device='all') {
		if (cblist === 'all') {
			...
			return this
		} else if (cblist instanceof String) {
			cblist = [cblist]
		}
		
		this._cbs.push((dat)=>{
			if (cblist.includes(dat.dataType)) {
				cb(dat)
			}
		})
